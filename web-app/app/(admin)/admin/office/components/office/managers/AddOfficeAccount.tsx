...
import { addOfficeAccountSchema, AddOfficeAccountSchemaType } from '@/lib/api/data/zod/schemas'
...
const AddOfficeAccount = ({ mutate,officeId }: Props) => {
    ...
    const defaultValues: AddOfficeAccountSchemaType = {
    ...
    const methods = useForm<AddOfficeAccountSchemaType>({
        resolver: zodResolver(addOfficeAccountSchema),
        defaultValues: defaultValues
    });

    const onSubmit = async (data: AddOfficeAccountSchemaType) => {
        const formData = data;
        unset(formData, "confirm_password");
        const promise = dataMutate(endpoints.admin.office.accounts.add(officeId), METHODS.POST, formData)
    ...
    return (
        <>
            ...
            >
                <IoIosPersonAdd size={20} /> إضافة حساب جديد
            </Button>
...
export default AddOfficeAccount