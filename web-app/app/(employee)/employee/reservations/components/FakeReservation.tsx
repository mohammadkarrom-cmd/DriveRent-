...
const FakeReservation = () => {
  const { data: reservations, error, mutate, isLoading } = useSWR(endpoints.reservations.fake.list, fetchApi);
  ...
  if (error) {
    if (error instanceof AxiosError) {
      if (error.status === 404) {
        return Empty({
          title: "لا يوجد اي حجوزات وهمية حاليا",
...
export default FakeReservation